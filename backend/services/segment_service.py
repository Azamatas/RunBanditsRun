...
def get_leaderboard(db: Session, segment_id: int, limit: int = 10) -> list[dict]:
    results = db.query(
        ...
        User, User.id == SegmentEffort.athlete_id
    ).filter(
        SegmentEffort.segment_id == segment_id
    ).group_by(
        SegmentEffort.athlete_id, User.username
    ).order_by(
        func.min(SegmentEffort.elapsed_time)
    ).limit(limit).all()

    return [
        {"athlete_id": r.athlete_id, "athlete_name": r.username, "best_time": r.best_time, "rank": idx + 1}
        for idx, r in enumerate(results)
    ]


def get_user_efforts(db: Session, segment_id: int, user_id: int) -> list[SegmentEffort]:
    return db.query(SegmentEffort).filter(
        SegmentEffort.segment_id == segment_id,
        SegmentEffort.athlete_id == user_id,
    ).order_by(SegmentEffort.started_at.desc()).all()